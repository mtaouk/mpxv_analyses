...
#### OLD
# Extract from super processed alignment
for i in {1..68}; do seqkit grep -r -p "^${i}_" /home/taouk/Monkeypox/alignments/consensus_pass_mafft_nogaps_masked_nodupref_noNs.aln > ${i}.aln; done
# Extract from alignment with Ns
for i in {1..68}; do seqkit grep -r -p "^${i}_" -p ON563414.3.masked /home/taouk/Monkeypox/alignments/consensus_pass_mafft_nogaps_masked_nodupref.aln > ${i}.aln; done
ls *.aln > input.txt
